    ...
    try {
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if($stmt->execute()){
                ...
                $stmt = $db->prepare('insert into sp_transcription (tranid,orderlist,amoun,shiping,operation,vat,netamoun,mil,updated_at ) values (?,?,?,?,?,?,?,?,?)');
                if ($stmt->execute([
                    $transid, $product, $amount, $shiping, 'PENDING', $vat, $netamount , $mil, $updated_at
                ])){
                    $object->RespCode = 200;
